...
public class SanPhamService {
    ...
    public Page<Product> searchProductsPaged(String keyword, Integer category, Pageable pageable) {
        // Lọc sản phẩm theo trạng thái (status = 0)
        List<Product> filteredProducts = sanPhamRepository.findByFilters(keyword, category)
                .stream()
                .filter(p -> p.getStatus() == 0)  // Lọc các sản phẩm còn bán
                .collect(Collectors.toList());

        // Áp dụng phân trang đúng cách với PageImpl
        int start = (int) pageable.getOffset();
        int end = Math.min((start + pageable.getPageSize()), filteredProducts.size());

        List<Product> pagedProducts = filteredProducts.subList(start, end);

        // Trả về PageImpl với số trang và kích thước trang chính xác
    ...
    // Tìm kiếm sản phẩm theo tên, danh mục, kích thước hoặc giá tăng dần/giảm dần
    public Page<Product> searchProductsAdvanced(String name, String category, String size, Boolean ascendingPrice, Pageable pageable) {
        return sanPhamRepository.findByFiltersAdvanced(name, category, size, ascendingPrice, pageable);
    }
